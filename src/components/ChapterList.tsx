      ...
      <header className="max-w-7xl mx-auto pt-8 px-4 md:px-8 w-full">
        ...
        <div className="mb-8 max-w-2xl">
          <div className="relative group rounded-full overflow-hidden">
            <span className="material-symbols-outlined absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#006b54] transition-colors z-10">search</span>
            <input 
              type="text" 
            ...
            {filteredChapters.map((chapter) => (
              <Link key={chapter.id} href={`/chapters/${chapter.slug}`} className="chapter-card bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 flex items-start gap-4 group shadow-sm hover:shadow-lg dark:hover:shadow-black/40 transition-all duration-250 ease-out hover:-translate-y-0.5">
                <div className="flex flex-col items-center flex-shrink-0">
                  ...
                  <span className="material-symbols-outlined text-primary/30 dark:text-[#cfaab7]/30 mt-3 text-xl">auto_stories</span>
                </div>
                <div className="flex-grow pt-1">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <span className="material-symbols-outlined text-[#006b54] opacity-0 group-hover:opacity-100 transition-all text-xl -mr-1">chevron_right</span>
                  </div>
